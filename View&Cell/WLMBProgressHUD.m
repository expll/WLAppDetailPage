...
@implementation WLMBProgressHUD
...
+ (WLMBProgressHUD *)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
	WLMBProgressHUD *hud = [[WLMBProgressHUD alloc] initWithView:view];
	[view addSubview:hud];
	[hud show:animated];
#if __has_feature(objc_arc)
	return hud;
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
	...
	for (UIView *v in [view subviews]) {
		if ([v isKindOfClass:[WLMBProgressHUD class]]) {
			viewToRemove = v;
		}
	}
	if (viewToRemove != nil) {
		WLMBProgressHUD *HUD = (WLMBProgressHUD *)viewToRemove;
